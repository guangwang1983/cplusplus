...
void KOScheduler::initExecutorSims()
{
    ...
    {
        int iOvernightPos = 0;
        double dOvernightPosPrice = 0;

        if(_mDailyOvernightPos.find(vProducts[i]) != _mDailyOvernightPos.end())
        {
            iOvernightPos = _mDailyOvernightPos[vProducts[i]].iPos;
            dOvernightPosPrice = _mDailyOvernightPos[vProducts[i]].dPrice;
        }
        ...
        _iPortfolioID = pNewExecutorSim->iaddPortfolio(_cSchedulerCfg.sDate, NULL, "", NULL, this);


        if(iOvernightPos != 0)
        {
            _pTradeSignalMerger->onFill(vProducts[i], iOvernightPos, dOvernightPosPrice, false);
            pNewExecutorSim->transferPosition(_iPortfolioID, iOvernightPos);
        }
        ...
        _iPortfolioID = pNewLiqExecutorSim->iaddPortfolio(_cSchedulerCfg.sDate, NULL, "", NULL, this);
