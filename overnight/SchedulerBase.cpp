...
bool SchedulerBase::postCommonInit()
{
    ...
    loadOvernightPos();
    writeOvernightResult();

    return bResult;
}

void SchedulerBase::writeOvernightResult()
{
    double dTotalPnL = 0;
    double dTotalVolume = 0;
    double dTotalNotional = 0;

    fstream fsProductOvernightFile;
    fsProductOvernightFile.open(_cSchedulerCfg.sLogPath + "/overnightpos.csv", fstream::out);
    if(!fsProductOvernightFile.is_open())
    {
        stringstream cStringStream;
        cStringStream << "Cannot write overnight file " << _cSchedulerCfg.sLogPath << "/overnightpos.csv";
        ErrorHandler::GetInstance()->newErrorMsg("0", "ALL", "ALL", cStringStream.str());            
    }

    for(unsigned int i = 0;i < _vContractQuoteDatas.size();i++)
    {
        double dProductMidPrice = _vContractQuoteDatas[i]->dLastValidMid;

        double dPnLInPrice = _vProductConsideration[i];
        long iProductTotalPos = _vProductPos[i] + _vProductLiquidationPos[i];
        ...
        double dPnLInDollar = dPnLInPrice * _vContractQuoteDatas[i]->dRateToDollar * _vContractQuoteDatas[i]->dContractSize;

        double dFee = _vProductVolume[i] * _vContractQuoteDatas[i]->dTradingFee * _vContractQuoteDatas[i]->dRateToDollar;
        double dFinalPnL = dPnLInDollar - dFee;

        fstream fsProductResultFile;
        fsProductResultFile.open(_cSchedulerCfg.sLogPath + "/" + _vContractQuoteDatas[i]->sProduct + "_ProductResult.out", fstream::out);
        if(fsProductResultFile.is_open())
        {
            fsProductResultFile << _cSchedulerCfg.sDate << ";" << dFinalPnL << ";" << _vProductVolume[i] << "\n";
        ...
        if(iProductTotalPos != 0)
        {
            if(fsProductOvernightFile.is_open())
            {
                fsProductOvernightFile << _vContractQuoteDatas[i]->sProduct << "," <<  iProductTotalPos << "," << dProductMidPrice << "\n";
            }
        }

        dTotalPnL = dTotalPnL + dFinalPnL;
        dTotalVolume = dTotalVolume + _vProductVolume[i];
        dTotalNotional = dTotalNotional + dTotalVolume * _vContractQuoteDatas[i]->dRateToDollar * _vContractQuoteDatas[i]->dContractSize;
    }

    fstream fsDailyResultFile;
    fsDailyResultFile.open(_cSchedulerCfg.sLogPath + "/DailyResult.out", fstream::out);

    if(fsDailyResultFile.is_open())
    {
        fsDailyResultFile << _cSchedulerCfg.sDate << ";" << dTotalPnL << ";" << dTotalVolume << ";" << dTotalNotional << "\n";
        fsDailyResultFile.close();
    }

    if(fsProductOvernightFile.is_open())
    {
        fsProductOvernightFile.close();
    }
}

void SchedulerBase::loadOvernightPos()
{
    fstream fsOvernightPosFile;
    fsOvernightPosFile.open(_cSchedulerCfg.sLogPath + "/overnightpos.csv", fstream::in);
    if(fsOvernightPosFile.is_open())
    {
        while(!fsOvernightPosFile.eof())
        {
            char sNewLine[512];
            fsOvernightPosFile.getline(sNewLine, sizeof(sNewLine));

            if(strlen(sNewLine) != 0 && sNewLine[0] != '#')
            {
                OvernightPos cNewOvernightPos;

                std::istringstream cOvernightPosStream(sNewLine);
                string sContract;
                string sPos;
                string sPrice;

                std::getline(cOvernightPosStream, sContract, ',');
                std::getline(cOvernightPosStream, sPos, ',');
                std::getline(cOvernightPosStream, sPrice, ',');

                cNewOvernightPos.iPos = atoi(sPos.c_str()); 
                cNewOvernightPos.dPrice = atof(sPrice.c_str());

                _mDailyOvernightPos[sContract] = cNewOvernightPos;
            }
        }
    }
}
    ...
    else
    {
        ...
        for(unsigned int i = 0; i < _vContractQuoteDatas.size(); ++i)
        {
               ...
               cCallTime < _vContractQuoteDatas[i]->cMarketCloseTime)
            {
                ...
                if(_vContractQuoteDatas[i]->bPriceValid == true)
                {
                    _vContractQuoteDatas[i]->dLastValidMid = _vContractQuoteDatas[i]->dWeightedMid;
                } 
            }
        }
...
//            {
//                iFound = sConfigFile.rfind("/", iFound-1);
...
//            }
            ...
            cerr << "sEngineRunTimePath " << sEngineRunTimePath << "\n";
...
{
    double dProductMidPrice = _vContractQuoteDatas[iProductIndex]->dLastValidMid;
                    ...
                    if(productPatientFiguresItr != _mProductFigurePatient.end())
                    {
                            ...
                            itrPatientFigures++)
                        {
                            ...
                            if(sFigureName.compare((*itr)->sFigureName) == 0)
                            {
                                ...
                                if(sFigureName.compare("EUR_EurECBInterestRateDecision") == 0 || sFigureName.compare("EUR_EurECBRateOnMainRefinancingOperations") == 0 || sFigureName.compare("EUR_EurECBrateonmainrefinancingoperations") == 0)
                                {
...
void SchedulerBase::registerSignalHandlers()
{
    ...
    max5SigAction.sa_handler = SchedulerBase::posPrint;
    ...
    max14SigAction.sa_handler = SchedulerBase::resetOrderStateHandler;
    ...
    minSigAction.sa_handler = SchedulerBase::printTheoTargetHandler;
    ...
    min1SigAction.sa_handler = SchedulerBase::printSignalHandler;
    ...
    min2SigAction.sa_handler = SchedulerBase::patientLiquidateProduct;
