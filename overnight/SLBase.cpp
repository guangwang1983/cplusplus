...
void SLBase::updateEngineStateOnTimer(KOEpochTime cCallTime)
{
    ...
    else if(_cQuotingPatLiqTime <= cCallTime && cCallTime < _cQuotingLimLiqTime)
    {
        ...
        {
            _cLogger << "Leaving overnight position. No liquidation \n";
            stringstream cStringStream;
            cStringStream << "Leaving overnight position. No liquidation";
            ErrorHandler::GetInstance()->newInfoMsg("0", _sEngineSlotName, vContractQuoteDatas[0]->sProduct, cStringStream.str());
        }
    }
...
void SLBase::checkFlatten()
{
    if(_sFlattenDatesFile != "")
    {
        _bFlattenToday = false;

        fstream ifsFlattenDatesFile;
        ifsFlattenDatesFile.open(_sFlattenDatesFile, fstream::in);

        if(ifsFlattenDatesFile.is_open())
        {
            while(!ifsFlattenDatesFile.eof())
            {
                char sNewLine[512];
                ifsFlattenDatesFile.getline(sNewLine, sizeof(sNewLine));

                if(strlen(sNewLine) != 0 && sNewLine[0] != '#')
                {
                    if(_sTodayDate == sNewLine)
                    {
                        _bFlattenToday = true;
                        break;
                    }
