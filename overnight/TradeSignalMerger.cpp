...
void TradeSignalMerger::aggregateAndSend(const string& sProduct)
{
    ...
    bool bprintPos = _mProductPrintPos[sProduct];
        ...
        itr++)
    {
        ...
        if(bprintPos)
        {
            cPosContentSStream << itr->sSlotName << " " << itr->iDesiredPos << " " << sProduct << "\n";
        }
...
//std::cerr << "Net external desired pos for " << sProduct << " is " << iTotalExternalDesiredQty << "\n";
    ...
    if(bprintPos)
    {
        fstream fsSlotPosFile;
        fsSlotPosFile.open("ConfigPosPrint_" + sProduct + ".dat", fstream::out);
        fsSlotPosFile << cPosContentSStream.str();
        fsSlotPosFile.close();
        _mProductPrintPos[sProduct] = false;
    }
}
