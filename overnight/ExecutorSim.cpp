...
int ExecutorSim::iaddPortfolio(string sDate, H5File* pH5File, string sOvernightPosFileName, CompType* pTransactionDataType, SchedulerBase* pSchedulerBase)
{
    int iPortfolioID = _vSimPortfolios.size();

    ExecutionPortfolio cNewPortfolio;
    cNewPortfolio.pH5File = pH5File;
    cNewPortfolio.sOvernightPosFileName = sOvernightPosFileName;
    ...
    int iFirstOvernightDate = 99999999;
    bool bFirstOvernightPosSeen = false;

    fstream fsOvernightPosFile;
    fsOvernightPosFile.open(sOvernightPosFileName.c_str(), fstream::in);
    if(fsOvernightPosFile.is_open())
    {
        while(!fsOvernightPosFile.eof())
        {
            ...
            {
                std::istringstream cNewLineStream(sNewLine);

                string sLineDate;
                std::getline(cNewLineStream, sLineDate, ',');
                string sOvernightPos;
                std::getline(cNewLineStream, sOvernightPos, ',');
                boost::gregorian::date cDate = boost::gregorian::from_undelimited_string(sLineDate);

                _mDailyOvernightPos[cDate] = atoi(sOvernightPos.c_str());

                if(bFirstOvernightPosSeen == false)
                {
                    iFirstOvernightDate = atoi(sLineDate.c_str());
                    bFirstOvernightPosSeen = true;
                }
            }
        }
    }

    boost::gregorian::date cPosDate;
    boost::gregorian::date cTodayDate = boost::gregorian::from_undelimited_string(sDate);
   
    if(cTodayDate.day_of_week().as_number() != 1)
    {
        cPosDate = cTodayDate - boost::gregorian::date_duration(1);
    }
    else
    {
        cPosDate = cTodayDate - boost::gregorian::date_duration(3);
    }

    while(atoi(to_iso_string(cPosDate).c_str()) >= iFirstOvernightDate)
    {
        map< boost::gregorian::date, int >::iterator itr = _mDailyOvernightPos.end();
        itr = _mDailyOvernightPos.find(cPosDate);

        _cLogger << "Trying to load overnight position from date " << to_iso_string(cPosDate) << "\n";
        if(itr != _mDailyOvernightPos.end())
        {
            int iOvernightPos = itr->second;
            cNewPortfolio.iPortfolioPosition = iOvernightPos;
            _cLogger << "Portfolio " << iPortfolioID << " Setting startiing position for " << sDate << " is " << cNewPortfolio.iPortfolioPosition << "\n";
            break;
        }
        else
        {
            _cLogger << "Cannot found overnigth position from date " << to_iso_string(cPosDate) << "\n";
        }

        int iTodayWeek = cPosDate.day_of_week().as_number();
        if(iTodayWeek != 1)
        {
            cPosDate = cPosDate - boost::gregorian::date_duration(1);
        }
        else
        {
            cPosDate = cPosDate - boost::gregorian::date_duration(3);
        }
    }
...
void ExecutorSim::writeTransactionsToFile(const string& sDate)
{
    ...
    for(unsigned int i = 0; i < _vPortfolioSimTransactions.size(); i++)
    {
        ...
        // write overnigth pos file
        bool bNewPosWritten = false;
        stringstream cNewOvernightPosStream;

        fstream fsOvernightPosFile;
        fsOvernightPosFile.open(_vSimPortfolios[i].sOvernightPosFileName.c_str(), fstream::in);
        if(fsOvernightPosFile.is_open())
        {
            _cLogger << " Writing overnight position for Portfolio " << i << " " << sDate << " " << _vSimPortfolios[i].iPortfolioPosition << "\n";
            while(!fsOvernightPosFile.eof())
            {
                char sNewLine[512];
                fsOvernightPosFile.getline(sNewLine, sizeof(sNewLine));
                ...
                if(strlen(sNewLine) != 0 && sNewLine[0] != '#')
                {
                    ...
                    if(sLineDate == sDate)
                    {
                        if(bNewPosWritten == false)
                        {
                            cNewOvernightPosStream << sDate << "," << _vSimPortfolios[i].iPortfolioPosition << "\n";
                            bNewPosWritten = true;
                        }
                    }
                    else if(atoi(sLineDate.c_str()) > atoi(sDate.c_str()))
                    {
                        if(bNewPosWritten == false)
                        {
                            cNewOvernightPosStream << sDate << "," << _vSimPortfolios[i].iPortfolioPosition << "\n";
                        ...
                        cNewOvernightPosStream << sNewLine << "\n"; 
                    }
                    else
                    {
                        cNewOvernightPosStream << sNewLine << "\n";
        ...
        char buffer[length];
        fsOvernightPosFile.rdbuf()->pubsetbuf(buffer, length);

        fsOvernightPosFile.open(_vSimPortfolios[i].sOvernightPosFileName.c_str(), fstream::out);
        if(fsOvernightPosFile.is_open())
        {
            fsOvernightPosFile << cNewOvernightPosStream.str();
        }
        else
        {
            _cLogger << "Failed to update overnight stat file " << _vSimPortfolios[i].sOvernightPosFileName << "\n";
        }
    }
}
