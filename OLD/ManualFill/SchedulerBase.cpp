...
void SchedulerBase::limitLiquidateProduct(int aSignal)
{
    ...
    _pInstance->liquidateProduct(true);
}
...
void SchedulerBase::fastLiquidateProduct(int aSignal)
{
    ...
    _pInstance->liquidateProduct(false);
}

void SchedulerBase::liquidateProduct(bool bIsLimit)
{
...
void SchedulerBase::addManualFill(int aSignal)
{
    stringstream cStringStream;
    cStringStream << "Received signal for add manual fill.";
    ErrorHandler::GetInstance()->newInfoMsg("0", "ALL", "ALL", cStringStream.str());

    _pInstance->processManualFills();
}

void SchedulerBase::processManualFills()
{
    ifstream ifsManualFillFile (_cSchedulerCfg.sManualFillFile);

    if(ifsManualFillFile.is_open())
    {
        while(!ifsManualFillFile.eof())
        {
            char sNewLine[256];
            ifsManualFillFile.getline(sNewLine, sizeof(sNewLine));
            ...
            if(strlen(sNewLine) > 0)
            {
                ...
                std::istringstream cFillLineStream(sNewLine);
                string sElement;

                std::getline(cFillLineStream, sElement, ';');
                sProduct = sElement;
                ...
                std::getline(cFillLineStream, sElement, ';');
                dPrice = atof(sElement.c_str());
                
                std::getline(cFillLineStream, sElement, ';');
                iQuantity = atoi(sElement.c_str());

                bool bManualFillMatched = false;

                for(vector<TradeEngineBasePtr>::iterator itr = _pInstance->_vTradeEngines.begin(); itr != _pInstance->_vTradeEngines.end(); itr++)
                {
                    if(sAccount == (*itr)->sgetEngineSlotName())
                    {
                        for(vector< boost::shared_ptr<ContractAccount> >::iterator accountItr = (*itr)->vContractAccount.begin();
                            accountItr != (*itr)->vContractAccount.end();
                            accountItr++)
                        {
                            if(sProduct == (*accountItr)->sgetAccountProductName())
                            {
                                bManualFillMatched = true;
                                (*itr)->manualFillHandler(sProduct, iQuantity, dPrice);                   
                                (*accountItr)->manualFillHandler(iQuantity, dPrice); 
                                _cPositionServerConnection.newFill(sProduct, sAccount, iQuantity);
                ...
                if(bManualFillMatched == false)
                {
                    stringstream cStringStream;
                    cStringStream << "Failed to match manual fill " << sNewLine;
                    ErrorHandler::GetInstance()->newErrorMsg("0", "ALL", "ALL", cStringStream.str()); 
                }
            }
        }
...
void SchedulerBase::registerSignalHandlers()
{
    ...
    max14SigAction.sa_handler = SchedulerBase::addManualFill;
    if (-1 == sigaction(SIGRTMAX-14, &max14SigAction, NULL))
    {
        stringstream cStringStream;
        cStringStream << "Failed to register for add manual fill signal handler.";
        ErrorHandler::GetInstance()->newErrorMsg("0", "ALL", "ALL", cStringStream.str());
    }
}
