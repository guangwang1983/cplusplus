...
SLSLUSFigure::SLSLUSFigure(const string& sEngineRunTimePath,
...
void SLSLUSFigure::readFromStream(istream& is)
{
...
void SLSLUSFigure::dayInit()
{
    setupLogger("SLSLUSFigure");
...
void SLSLUSFigure::receive(int iCID)
{
    SLBase::receive(iCID);
...
void SLSLUSFigure::updateStatistics(KOEpochTime cCallTime)
{
    if(cCallTime > _cTradingSwitchOffTime)
    {
        if(_bIsTrading == true)
        {
            _cLogger << "Figure Trading Switched off \n";
            _bIsTrading = false;
        }
    }
    ...
    if(_pQuoteInstrument->bgetEXMAValid() && _pQuoteInstrument->bgetWeightedStdevValid() && _pSignalInstrument->bgetEXMAValid() && _pSignalInstrument->bgetWeightedStdevValid())
    {
        ...
        if(_pProductInstrument->bgetWeightedStdevValid())
        {
            ...
            if(abs(dQuoteLeg) > 500 && abs(dSignalLeg) > 500)
            {
                _bIsBullishMom = true;
                _bIsBearishMom = true;
...
void SLSLUSFigure::writeSpreadLog()
{
...
bool SLSLUSFigure::bcheckAllProductsReady()
{
    bool bResult = false;
    if(_pQuoteInstrument != NULL && _pSignalInstrument != NULL && _pProductInstrument != NULL)
    {
...
void SLSLUSFigure::wakeup(KOEpochTime cCallTime)
{
    SLBase::wakeup(cCallTime);
}

void SLSLUSFigure::loadRollDelta()
{
...
void SLSLUSFigure::saveOvernightStats(bool bRemoveToday)
{
...
void SLSLUSFigure::loadOvernightStats()
{
...
void SLSLUSFigure::figureCall(KOEpochTime cCallTime, KOEpochTime cEventTime, const string& sFigureName, FigureAction::options eFigureAction, double dForecast, double dActual, bool bReleased)
{
    if(sFigureName == "USD_NonfarmPayrolls" || sFigureName == "USD_FedInterestRateDecision" || sFigureName == "USD_FOMCMinutes")
    {
        _bIsTrading = true;
        _cTradingSwitchOffTime = cEventTime + KOEpochTime(7200, 0);
        _cLogger << "Figure Trading Started for " << sFigureName << ". Switch off time is " << _cTradingSwitchOffTime.igetPrintable() << "\n";
    }
    else if(sFigureName == "EUR_EUClose")
    {
        _bIsTrading = true;
        _cTradingSwitchOffTime = cEventTime + KOEpochTime(14400, 0);
        _cLogger << "Evening trading started \n";
    }

    SLBase::figureCall(cCallTime, cEventTime, sFigureName, eFigureAction, dForecast, dActual, bReleased);
}
