...
void Product::setRollParams(double dRollAdjust, double dRollDelta)
{
    _dRollAdjust = dRollAdjust;
    _dRollDelta = dRollDelta;
}
...
void Product::useWeightedStdev(long iStdevLength, bool bDailyUpdate, KOEpochTime cUpdateTime, int iUpdateFreqSeconds, KOEpochTime cWeightedStdevStartTime, KOEpochTime cWeightedStdevEndTime, bool bDetrend)
{
    ...
    _bWeightedStdevDetrend = bDetrend;
...
void Product::useStaticStdev()
{
    _bUseStaticStdev = true;
}

void Product::setStaticStdev(double dStaticStdev)
{
    _dStaticStdev = dStaticStdev;
}

double Product::dgetStaticStdev()
{
    return _dStaticStdev;
}
...
{
    if(_bWeightedStdevDetrend == false)
    {
        if(_pWeightedStdev != NULL)
        {
...
void Product::wakeup(KOEpochTime cT)
{
	...
	if(_bUseWeightedStdev)
	{
		if(_bWeightedStdevDailyUpdate)
		{
			...
			{
                if(_bWeightedStdevDetrend == true)
                {
                    if(bgetEXMAValid())
                    {
                        _pWeightedStdev->dnewData(dgetWeightedMid() - dgetEXMA());
                    }
                }
                else
                {
