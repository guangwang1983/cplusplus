...
SL3L_FI_Filter::SL3L_FI_Filter(const string& sEngineRunTimePath,
...
:SLBase(sEngineRunTimePath, "SL3L_FI_Filter", sEngineSlotName, cTradingStartTime, cTradingEndTime, pScheduler, sTodayDate, sSimType, cSlotFirstWakeupCallTime),
 ...
 _bUpdateStats(true)
{
    ...
    _pFilterFrontInstrument = NULL;
    _pFilterBackInstrument = NULL;
...
SL3L_FI_Filter::~SL3L_FI_Filter()
{
    ...
    if(_pFilterFrontInstrument != NULL)
    {
        delete _pFilterFrontInstrument;
    }

    if(_pFilterBackInstrument != NULL)
    {
        delete _pFilterBackInstrument;
    }
...
void SL3L_FI_Filter::readFromStream(istream& is)
{
    ...
    while(!is.eof())
    {
        ...
        else if(sParamName.compare("FrontFilter") == 0)
        {
            string sValue;
            std::getline(cParamStream, sValue, ':');
            ...
            if(sValue == "1")
            {
                _bUseFrontFilter = true;
            }
            else
            {
                _bUseFrontFilter = false;
            }
        }
        else if(sParamName.compare("BackFilter") == 0)
        {
            string sValue;
            std::getline(cParamStream, sValue, ':');
            ...
            if(sValue == "1")
            {
                _bUseBackFilter = true;
            }
            else
            {
                _bUseBackFilter = false;
            }
        }
    }
...
void SL3L_FI_Filter::dayInit()
{
    setupLogger("SL3L_FI_Filter");
    ...
    if(_pSpreadFrontInstrument == NULL)
    {
        ...
        _pFilterFrontInstrument = new Instrument(vContractQuoteDatas[3]->sProduct, vContractQuoteDatas[3]->iCID, vContractQuoteDatas[3]->eInstrumentType, vContractQuoteDatas[3]->dTickSize, vContractQuoteDatas[1]->iMaxSpreadWidth, _bUseRealSignalPrice);
        _pFilterFrontInstrument->useEXMA(3600, false, KOEpochTime(0,0) , 1, _cTradingStartTime, _cTradingEndTime);

        _pFilterBackInstrument = new Instrument(vContractQuoteDatas[4]->sProduct, vContractQuoteDatas[4]->iCID, vContractQuoteDatas[4]->eInstrumentType, vContractQuoteDatas[4]->dTickSize, vContractQuoteDatas[1]->iMaxSpreadWidth, _bUseRealSignalPrice);
        _pFilterBackInstrument->useEXMA(3600, false, KOEpochTime(0,0) , 1, _cTradingStartTime, _cTradingEndTime);
...
void SL3L_FI_Filter::dayRun()
{
    ...
    _pFilterFrontInstrument->newMarketUpdate(vContractQuoteDatas[3]);
    _pFilterBackInstrument->newMarketUpdate(vContractQuoteDatas[4]);
...
void SL3L_FI_Filter::dayStop()
{
    ...
    _pFilterFrontInstrument->eodReset();
    _pFilterBackInstrument->eodReset();
...
void SL3L_FI_Filter::receive(int iCID)
{
    SLBase::receive(iCID);
...
*/
    ...
    else if(iUpdateIndex == 3)
    {
        _pFilterFrontInstrument->newMarketUpdate(vContractQuoteDatas[iUpdateIndex]);
    }
    else if(iUpdateIndex == 4)
    {
        _pFilterBackInstrument->newMarketUpdate(vContractQuoteDatas[iUpdateIndex]);
    }
}

void SL3L_FI_Filter::updateStatistics(KOEpochTime cCallTime)
{
    ...
    _pFilterFrontInstrument->wakeup(cCallTime);
    _pFilterBackInstrument->wakeup(cCallTime);
    ...
    if(_pSpreadFrontInstrument->bgetEXMAValid() &&
       ...
       _pFilterFrontInstrument->bgetEXMAValid() &&
       _pFilterBackInstrument->bgetEXMAValid())
    {
       ...
       _pSpreadInstrument->bgetWeightedStdevValid())
    {
        ...
        if(_pProductInstrument->bgetWeightedStdevValid())
        {
            ...
            double dFilterFrontLeg = (_pFilterFrontInstrument->dgetWeightedMid() - _pFilterFrontInstrument->dgetEXMA()) / _pFilterFrontInstrument->dgetTickSize();
            double dFilterBackLeg = (_pFilterBackInstrument->dgetWeightedMid() - _pFilterBackInstrument->dgetEXMA()) / _pFilterBackInstrument->dgetTickSize();
            ...
            {
                _cLogger << "Bullish Trend. No short \n";
            }

            if(_bIsBearishMom == true)
            {
                _cLogger << "Bearish Trend. No long \n";
            }

            if(_bUseFrontFilter)
            {
                if(dQuoteLeg > 3 && dFilterFrontLeg < -30)
                {
                    _bIsBullishBreak = true;
                    _bIsBearishBreak = false;
                }
                else if(dQuoteLeg < -3 && dFilterFrontLeg > 30)
                {
                    _bIsBullishBreak = false;
                    _bIsBearishBreak = true;
                }
                else
                {
                    if(-1 < dQuoteLeg && dQuoteLeg < 1 && -5 < dFilterFrontLeg && dFilterFrontLeg < 5)
                    {
                        _bIsBullishBreak = false;
                        _bIsBearishBreak = false;
                    }
            ...
            if(_bUseBackFilter)
            {
                if(dQuoteLeg > 3 && dFilterBackLeg > 30)
                {
                    _bIsBullishBreak = _bIsBullishBreak || true;
                    _bIsBearishBreak = _bIsBearishBreak || false;
                }
                else if(dQuoteLeg < -3 && dFilterBackLeg < -30)
                {
                    _bIsBullishBreak = _bIsBullishBreak || false;
                    _bIsBearishBreak = _bIsBearishBreak || true;
                }
                else
                {
                    if(-1 < dQuoteLeg && dQuoteLeg < 1 && -5 < dFilterFrontLeg && dFilterFrontLeg < 5)
            ...
            if(_bIsBullishBreak == true)
            {
                _cLogger << "Bullish correlation break. No short \n";
            }

            if(_bIsBearishBreak == true)
            {
                _cLogger << "Bearish correlation break. No long \n";
            }
            ...
            if(_bIOC == true)
            {
                ...
                _cLogger << "|" << _pProductInstrument->dgetBid();
            }
            else
            {
                _cLogger << "|" << _dSpreadIndicator;
            ...
            _cLogger << "|" << _pFilterFrontInstrument->dgetWeightedMid();
            _cLogger << "|" << _pFilterFrontInstrument->dgetEXMA();
            _cLogger << "|" << _pFilterBackInstrument->dgetWeightedMid();
            _cLogger << "|" << _pFilterBackInstrument->dgetEXMA() << std::endl;
...
void SL3L_FI_Filter::writeSpreadLog()
{
...
bool SL3L_FI_Filter::bcheckAllProductsReady()
{
   bool bResult = false;
       ...
       _pProductInstrument != NULL)
    {
        if(_pQuoteInstrument->bgetPriceValid() && _pSpreadFrontInstrument->bgetPriceValid() && _pSpreadBackInstrument->bgetPriceValid())
        {
            ...
            if(_bUseFrontFilter)
            {
                bResult = bResult && _pFilterFrontInstrument->bgetPriceValid();
            }

            if(_bUseBackFilter)
            {
                bResult = bResult && _pFilterBackInstrument->bgetPriceValid();
            }
        }
...
void SL3L_FI_Filter::wakeup(KOEpochTime cCallTime)
{
    SLBase::wakeup(cCallTime);
}

void SL3L_FI_Filter::loadRollDelta()
{
...
void SL3L_FI_Filter::saveOvernightStats(bool bRemoveToday)
{
    ...
    for(map< boost::gregorian::date, boost::shared_ptr<DailyStat> >::iterator itr = _mDailyStats.begin(); itr != _mDailyStats.end(); ++itr)
    {
        if(itr->first != cTodayDate)
        {
            if(itr->first > cTodayDate && !bCurrentDayWritten)
            {
                if(_pQuoteInstrument->dgetEXMA() > 0.001 && _pSpreadFrontInstrument->dgetEXMA() > 0.001 && _pSpreadBackInstrument->dgetEXMA() > 0.001 && bRemoveToday == false)
                {
                    ...
                    cStringStream << _pSpreadInstrument->dgetWeightedStdevEXMA() << ","
                                  ...
                                  << _dLastSpreadBackMid << ","
                                  << _pFilterFrontInstrument->dgetEXMA() << ","
                                  << _pFilterFrontInstrument->igetEXMANumDataPoints() << ","
                                  << _pFilterBackInstrument->dgetEXMA() << ","
                                  << _pFilterBackInstrument->igetEXMANumDataPoints() << "\n";
                }
            ...
            cStringStream << itr->second->dSpreadInstrumentWeightedStdevEXMA << ","
                          ...
                          << itr->second->dFilterFrontEXMA << ","
                          << itr->second->iFilterFrontEXMANumDataPoints << ","
                          << itr->second->dFilterBackEXMA << ","
                          << itr->second->iFilterBackEXMANumDataPoints << "\n";
        }
    }
    ...
    pTodayStat->dFilterFrontEXMA = _pFilterFrontInstrument->dgetEXMA();
    pTodayStat->iFilterFrontEXMANumDataPoints = _pFilterFrontInstrument->igetEXMANumDataPoints();
    pTodayStat->dFilterBackEXMA = _pFilterBackInstrument->dgetEXMA();
    pTodayStat->iFilterBackEXMANumDataPoints = _pFilterBackInstrument->igetEXMANumDataPoints();

    _mDailyStats[cTodayDate] = pTodayStat;
}

void SL3L_FI_Filter::loadOvernightStats()
{
		...
		while(!ifsOvernightStatFile.eof())
		{
			...
			if(strlen(sNewLine) != 0 && sNewLine[0] != '#')
			{
                ...
                if(bDateIsValid == true)
                {
                    ...
                    try
                    {
                        pDailyStat->dFilterFrontEXMA = atof(sElement.c_str());
                        bStatValid = bStatValid && true;
                    }
                    catch(exception e)
                    {
                    ...
                    try
                    {
                        pDailyStat->iFilterFrontEXMANumDataPoints = atoi(sElement.c_str());
                        bStatValid = bStatValid && true;
                    }
                    catch(exception e)
                    {
                    ...
                    try
                    {
                        pDailyStat->dFilterBackEXMA = atof(sElement.c_str());
                        bStatValid = bStatValid && true;
                    }
                    catch(exception e)
                    {
                    ...
                    try
                    {
                        pDailyStat->iFilterBackEXMANumDataPoints = atoi(sElement.c_str());
                        bStatValid = bStatValid && true;
                    }
                    catch(exception e)
                    {
	...
	if(itr != _mDailyStats.end())
	{
      ...
      _cLogger << "with " << itr->second->iSpreadInstrumentWeightedStdevNumDataPoints << " data points \n";
       ...
       _cLogger << "with " << itr->second->iProductWeightedStdevNumDataPoints << " data points \n";
        ...
        _cLogger << "Assign dFilterFrontEXMA: " << itr->second->dFilterFrontEXMA << "\n";
        _cLogger << "with " << itr->second->iFilterFrontEXMANumDataPoints << " data points " << "\n";
        _pFilterFrontInstrument->setNewEXMA(itr->second->dFilterFrontEXMA, itr->second->iFilterFrontEXMANumDataPoints);

        _cLogger << "Assign dFilterBackEXMA: " << itr->second->dFilterBackEXMA << "\n";
        _cLogger << "with " << itr->second->iFilterBackEXMANumDataPoints << " data points " << "\n";
        _pFilterBackInstrument->setNewEXMA(itr->second->dFilterBackEXMA, itr->second->iFilterBackEXMANumDataPoints);
	}
}
