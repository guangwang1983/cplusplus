        ...
        else if(charExecType == '8') // order rejected
        {
            ...
            if(pOrderToBeUpdated->_eOrderState == KOOrder::PENDINGCREATION)
            {
                ...
                if(_vLastOrderError[iProductIdx] != cStringStream.str())
                {
                    ...
                    {
                        _vLastOrderError[iProductIdx] = cStringStream.str();
                        if(sRejectReason.find("Price Protection") == std::string::npos && sRejectReason.find("Market move") == std::string::npos && sRejectReason.find("last look") == std::string::npos && sRejectReason.find("outside Tolerance") == std::string::npos && sRejectReason.find("invalid rate") == std::string::npos) 
                        {
                            ErrorHandler::GetInstance()->newErrorMsg("0", pOrderToBeUpdated->_sAccount, pOrderToBeUpdated->sgetOrderProductName(), cStringStream.str());
                        }
                    }
        ...
        if(bOrderFound == true)
        {
            ...
            if(pOrderToBeUpdated->_eOrderState == KOOrder::PENDINGCREATION)
            {
                ...
                if(_vLastOrderError[iProductIdx] != cStringStream.str())
                {
                    ...
                    {
                        _vLastOrderError[iProductIdx] = cStringStream.str();
                        if(sText.find("Price Protection") == std::string::npos && sText.find("Market move") == std::string::npos && sText.find("last look") == std::string::npos && sText.find("outside Tolerance") == std::string::npos && sText.find("invalid rate") == std::string::npos)
                        {
                            ErrorHandler::GetInstance()->newErrorMsg("0", pOrderToBeUpdated->_sAccount, pOrderToBeUpdated->sgetOrderProductName(), cStringStream.str());
                        }
                    }
