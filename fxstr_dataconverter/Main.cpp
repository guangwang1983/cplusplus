			...
			if(strlen(sNewLine) != 0 && sNewLine[0] != '#')
			{
				...
				else if(eFileFormat != UNKNOWN)
				{
                    ...
                    if((sLineAction == "Trade" || sLineAction == "Quote") && sTradeQualifier.find("IND[MKT_ST_IND]") == std::string::npos && sTradeQualifier.find("Exhausted Bid and Ask") == std::string::npos && sTradeQualifier.find("   [MKT_ST_IND]") == std::string::npos)
                    {
                        if(sTimeStamp != "")
                        {
//                            20240917-00:00:00.008910
//                            sDate.substr(0,4) + "-" + sDate.substr(4,2) + "-" + sDate.substr(6,2) + "T00:00:01Z000000";
                            string sUTC = sTimeStamp.substr(0,4) + "-" + sTimeStamp.substr(4,2) + "-" + sTimeStamp.substr(6,2) + "T" + sTimeStamp.substr(9,8) + "Z";
                            iCurrentTime = iFromStringToEpoch(sUTC) * 1000000 + atoi(sTimeStamp.substr(18,6).c_str());
...
//                            iCurrentTime = iFromStringToEpoch(sTimeStamp.substr(0,19) + "Z") * 1000000 + atoi(sTimeStamp.substr(20,6).c_str());
                            ...
                            {
                                bIgnoreUpdate = true;
                                cerr << "ERROR: Ignoring updates with backward time stamp " << iCurrentTime << " last update time " << iLastUpdateTime << "\n";
                            }
